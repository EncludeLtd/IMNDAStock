...
BuildingChecks.prototype.onSuccessSfdcBuildingChecks = function(records)
{
    ...
    if (records.length > 0)
    {
	    ...
	    $j.each(records, function(i, buildingCheck) 
	    {
	    	...
	    	{
	    		if (currentArea != "") nestedList += "</ul>";
	    		currentArea = buildingCheck.Area__c;
	    		var checkTime = buildingCheck.Time_of_Check__c.substring (11,16);
	    		nestedList += '<li id="li' + buildingCheck.Area__c.replace(/ /g,"") + '">'+checkTime + " " + buildingCheck.Area__c;
	    		nestedList += '<ul data-role="listview" id="ul' + i + '">';
	    	};
	    ...
	    $j.each(records, function(i, buildingCheck) {
	    	that.addClickHandlersToList (i, buildingCheck);
	    });
...
BuildingChecks.prototype.displayOneCheck = function (i, buildingCheck)
{
	var that=this;
    ...
    newLi += "<label for='Hazard" + i + "'>Hazard</label>";
    newLi += "<input type='radio' id='Hazard" + i + "' data-theme='c' name='n" + i + "' value='Hazard' ";
    if (buildingCheck.Result__c == 'Hazard') {newLi += "checked >"} else {newLi += ">"};
...
//        logToConsole("OK Button " + i + " in buildingCheck page clicked");
        buildingCheck.Result__c = 'OK';
        that.updateRecord (buildingCheck, onUpdateError);
        that.removeItem (i, buildingCheck.Area__c);
    });
    $j('#Issue'+i).click(function(event) {
//        logToConsole("Issue Button " + i + " in buildingCheck page clicked");
...
//        logToConsole("Hazard Button " + i + " in buildingCheck page clicked");
        buildingCheck.Result__c = 'Hazard';
...
BuildingChecks.prototype.removeItem = function (itemIndex, area)
{
//	logToConsole ('BuildingChecks.prototype.removeItem ' + area);
	var $j = jQuery.noConflict();
	var that=this;
	var areaID = '#li' + area.replace(/ /g,"");
...
//    cordova.require("salesforce/util/logger").logToConsole("onUpdateError: " + JSON.stringify(error) );
